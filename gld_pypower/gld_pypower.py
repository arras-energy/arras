...
class Model:
    ...
    def __init__(self,data:str|TypeVar('io.StringIO')|dict):
        """Access GridLAB-D PyPower model

        Arguments:

            `data`: JSON string, filename, or dict of model data
        """
        ...
        self.name = os.path.splitext(os.path.split(data["globals"]["modelname"]["value"])[1])[0]
        self.data = data
        self.results = {}
        self.modified = False
    ...
    def del_object(self,obj:str,on_ref:str='error',ignore_errors:bool=False):
        """Delete object
        ...
        Arguments:
            ...
            `on_ref`: how handle objects that refer to deleted object,
            e.g., 'delete', 'ignore' or 'error'


            `ignore_errors`: enable ignoring of errors

        Returns:

            `dict`: data from deleted object
        """
        if isinstance(obj,list):
            return [del_object(x,on_ref) for x in obj]
        if obj not in self.data["objects"] and not ignore_errors:
    ...
    def mod_object(self,obj:str,**kwargs):
        ...
        Arguments:
            ...
            `kwargs`: properties and values to modify

        Returns:
        
            `dict`: data from modified object
        """
    ...
    def save(self,filename:str=None,**kwargs):
        """Save model to file
        """
        with open(filename if filename else self.name+".json","w") as fh:
            json.dump(self.data,fh,**kwargs)
        self.modified = False

    def run(self,
            filename:str=None,
            binary:bool="GLD_ETC" in os.environ,
            exception:bool=True,
            autosave:bool=True,
            *args,
            **kwargs):
        ...
        Arguments:
            ...
            `name`: file name to load (default is ./${modelname}.glm)

            `binary`: enable use of gridlabd binary with environment

            `exception`: enable use of exceptions on errors

            `autosave`: enable autosave of modified models

            `args`: command line arguments

            `kwargs`: global definitions

        Returns:

            int,list[str],list[str]: exit code, stdout, and stderr
        """
        if not filename:
    ...
    def graphSpectral(self,refresh:bool=False) -> tuple[float]:
        ...
        Returns:
            ...
            `tuple`: (E,U,K) where E is the eigenvalues, U is the eigenvectors, and K is the number of networks found
        """
        self.assert_module("pypower")
...
def main(*args,**kwargs):
    ...
    raise NotImplementedError("gld_pypower.main()")

if __name__ == "__main__":

    # self-test only works if the source file is run from the current working directory
    if os.path.split(__file__)[0] == os.getcwd():
        ...
        if runtime:
            testEq(test.run(filename="example.glm",savefile="example.json"),(0,[''],['']),"initial run test failed")
